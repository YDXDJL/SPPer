...
> 文档基准日期：2026-08-13
> 项目根目录：`D:\Vibecoding Projects\SPPer`
...
| RGB 硬件网关 | `0.3.1` | ESP32 | `main/` |
| 救生员手环 | `0.3.1` | HU-087、ESP32-S3 | `band/` |
| 泳者安全终端 | `0.5.5` | ESP32-C3 | `swimmer/` |
...
├─ yolo/
│  ├─ FIXED_DATA/            演示原视频与跟踪配置
│  └─ tracking/
│     └─ output_configured/  已生成的 JSONL、事件、摘要和标注视频
...
`board/`、`deliverables/`、`.codex_board_work/` 等目录属于辅助制作、交付或工作区内容，
不参与当前电脑端运行时的核心链路。
...
- **触发救生装置**：在线泳者设备可在任何视频和锁存状态下手动触发；重复触发会
  重新开始完整舵机动作，上一条命令未完成时由后端等待并顺序执行；
- **复位救生装置**：设备在线时可随时执行；未锁存时也是安全的幂等操作，设备会
  确认成功并回到 180°。
